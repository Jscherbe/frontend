...
export class ComponentInitializer {
  ...
  updateElements(config) {
    const { update, key, withData, context } = config;
    const elementsWithData = this.queryAllInitialized(key, withData, context);
    elementsWithData.forEach(elementWithData => update(elementWithData, this));
  }
  /**
   ...
   * @return {String}
   */  
  attributeSelectorInitialized(key) {
    return `${ this.attributeSelector(key) }[${ this.getAttribute("init") }]`;
  }
  /**
   * Internal helper to query and map elements.
   * @private
   */
  queryElements(selector, key, withData, context = document, isInitial = false) {
    ...
    return elements.map((element) => {
      ...
      if (isInitial) {
        item.initialize = () => this.initializeElement(element);
      }
      return item;
    });
  }
  ...
  queryAllInitial(key, withData, context = document) {
    return this.queryElements(
      this.attributeSelectorInitial(key), 
      key, 
      withData, 
      context, 
      true
   ...
   */
  queryAllInitialized(key, withData, context = document) {
    return this.queryElements(
      this.attributeSelectorInitialized(key), 
      key, 
      withData, 
      context, 
      false
